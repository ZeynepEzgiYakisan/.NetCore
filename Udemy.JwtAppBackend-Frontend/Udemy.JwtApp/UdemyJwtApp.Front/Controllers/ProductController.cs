    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            if (token != null)
            {
                ...
                var response = await client.GetAsync("http://localhost:60513/api/products");
        ...
        public async Task<IActionResult> Remove(int id)
        {
            var token = User.Claims.FirstOrDefault(x => x.Type == "accesToken")?.Value;
            {
                ...
                await client.DeleteAsync($"http://localhost:60513/api/products/{id}");
            }
        ...
        public async Task<IActionResult> Create()
        {
            ...
            if (token != null)
            {
                ...
                var response = await client.GetAsync($"http://localhost:60513/api/categories");
        ...
        {
            var data = TempData["Categoris"]?.ToString();
            if(data != null)
            {
            ...
            if(ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.PostAsync("http://localhost:60513/api/products", content);
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            if (token != null)
            {
                ...
                var responseProduct = await client.GetAsync($"http://localhost:60513/api/products/{id}");
                ...
                if (responseProduct.IsSuccessStatusCode)
                {
                    ...
                    var responseCategory = await client.GetAsync($"http://localhost:60513/api/categories");
        ...
        public async Task<IActionResult> Update(UpdateProductModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var token = User.Claims.FirstOrDefault(x => x.Type == "accesToken")?.Value;
                {
                    ...
                    var response = await client.PutAsync("http://localhost:60513/api/products", content);
